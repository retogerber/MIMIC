...
pairs_plot <- function(input_csv_files, vis_type, quantile_type="quantile50", return_missing=FALSE){
  ...
  pltls <- list()
  ismissing <- rep(FALSE, dim(df)[1])
  if("euclidean_distance_centroids" %in% colnames(df) & "dice_score" %in% colnames(df)){
  ismissing <- ismissing | is.na(df$euclidean_distance_centroids) | is.na(df$dice_score)
  masks_plt <- 
  ...
  ismissing <- ismissing | is.na(df[[axis_type]]) | is.na(df[[yaxis_type]])
  landmarks_plt <- 
    plotly::add_markers(base, 
  ...
  ismissing <- ismissing | is.na(df[[xaxis_type]]) | is.na(df[[yaxis_type]])
  regions_plt <- 
    plotly::add_markers(base, 
  ...
  if(length(pltls)>0){
    ...
    if (return_missing) {
       return(list(fig, df$sample[ismissing]))
    } else{
    return(fig)
    }
  }else{
...
if(length(tmp[[2]])>0){
    cat("\n\n")
    knitr::kable(data.frame(missing=tmp[[2]]))
}
cat(sprintf("\n\n### %s\n\n","mean"))
