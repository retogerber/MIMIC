...
def get_regions_sam_prefilter(img, mask, sam, min_area=24**2,max_area=512**2, quantile=0.05, dilation_kernel_size=20, opening_kernel_radius=5, erosion_kernel_size=50):
    ...
    logging.info(f"mask_exp shape: {mask_exp.shape}")
    logging.info(f"Number of pixels in mask: {np.sum(mask_exp>0)}")
    logging.info(f"mbb: {mbb}")
    logging.info(f"first subset: {img[mbb[1]:mbb[1]+mbb[3],mbb[0]:mbb[0]+mbb[2]].shape}")
    logging.info(f"second subset: {mask_exp[mbb[1]:mbb[1]+mbb[3],mbb[0]:mbb[0]+mbb[2]].shape}")
    ...
    logging.info(f"background_pixels shape: {background_pixels.shape}")
...
logging.info(f"bounding box for region extraction: {bb31}")
