...
input_csv_files <- "/home/retger/IMC/data/complete_analysis_imc_workflow/imc_to_ims_workflow/results/cirrhosis_TMA/data/registration_metric/cirrhosis_TMA_reg_metrics_combined.csv"
...
lookupdf <- data.frame(
  metric = c(
    ...
    "IMC_to_postIMC_quantile50_error",
...
dice_limit <- 0.5
tmpdfc <- dfc
tmpdfc$value[tmpdfc$metric_type=="distance"][tmpdfc$value[tmpdfc$metric_type=="distance"]>dist_limit] <- dist_limit
tmpdfc$value[tmpdfc$metric_type=="dice"][tmpdfc$value[tmpdfc$metric_type=="dice"]<dice_limit] <- dice_limit

ggplot() +
  geom_tile(data = tmpdfc, aes(metric, sample, fill = evaluation_type)) +
  scale_fill_manual(values = c(region = "gray92", landmark = "white")) +
  new_scale_color() +
  geom_point(
    data = dplyr::filter(tmpdfc, metric_type == "distance"),
    aes(metric, sample, color = value, size = confidence_value_scaled)
  ) +
  scale_color_viridis_c(option = "mako", limits = c(0, 10), na.value = "gray95", direction = -1, breaks=c(seq(0,dist_limit,2)), labels=c(seq(0,dist_limit-2,2),paste0(">",dist_limit))) +
  new_scale_color() +
  geom_point(
    data = dplyr::filter(tmpdfc, metric_type == "dice"),
    aes(metric, sample, color = value, size = confidence_value_scaled)
  ) +
  scale_color_viridis_c(option = "viridis", limits = c(dice_limit, 1), na.value = "gray90", breaks=seq(dice_limit,1,0.1), labels=c(paste0("<",dice_limit),seq(dice_limit+0.1,1,0.1))) +
  facet_grid(cols = vars(type), scales = "free_x", space = "free_x") +
  scale_x_discrete(labels = xlabels) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(colour = "Dice\ncoefficient", size = "Confidence\nguess", colour_new = "Distance", x = "", y = "", fill = "Evaluation\nType", title = paste0("Summary of registration metrics for ", project_name)) 
```
...
```{r, eval=FALSE}
ggplot(df) +
  geom_point(aes(x=IMC_to_preIMC_quantile50_error, y=proportion_area_no_tissue_preIMC_IMC_beforereg/proportion_area_no_tissue_preIMC_IMC_afterreg, color=proportion_area_no_tissue_preIMC_IMC_beforereg)) +
  scale_color_viridis_c(option = "mako", na.value = "gray95")

df |>
  dplyr::filter(IMC_to_preIMC_quantile50_error > 15) |>
  dplyr::select(sample, proportion_area_no_tissue_preIMC_IMC_beforereg)

df |>
  tidyr::pivot_longer(cols = c("proportion_area_no_tissue_preIMC_IMC_beforereg", "proportion_area_no_tissue_preIMC_IMC_afterreg")) |>
ggplot() +
  geom_point(aes(x=name, y=value)) +
  geom_line(aes(x=name, y=value, group=sample)) 
```
