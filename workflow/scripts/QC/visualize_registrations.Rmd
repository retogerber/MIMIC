...
if("IMS_to_postIMS" %in% snakemake@params[["QC_steps"]]){
  ...
  to_keep_df$keep_IMS_to_postIMS <- rep(NA, length(df$sample))
}
...
```{r metrics_summary_plot, warning=FALSE, message=FALSE, fig.width=10,fig.height=5+0.5*length(unique(dfc$sample))}
...
```{r metrics_summary_plot_bin, fig.width=12, fig.height=3+0.5*length(unique(dfc$sample))}
...
```{r, fig.width=10, fig.height=5, include=to_include_sitk}
